...
if st.session_state.data is not None:
    ...
    with st.sidebar:
        ...
        # Show file uploader directly in the sidebar
        st.markdown("<h4 style='color: green; text-align: center;'>Upload Stock Data</h4>", unsafe_allow_html=True)
        stock_data = upload_stock_data()  # Upload stock data directly

    # Main section for displaying stock analysis if stock data has been uploaded
    if stock_data is not None:
        st.markdown("<h4 style='color: green; text-align: center;'>Stock/Inventory Analysis</h4>", unsafe_allow_html=True)
        st.markdown("---")

        # Perform analysis only if stock data has been uploaded

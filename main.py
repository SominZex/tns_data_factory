...
#from streamlit_extras import export_pdf
# import base64
...
if st.session_state.show_uploader:
    ...
    if uploaded_file is not None:
        ...
        # Parse the 'orderDate' column in 'dd-mm-yyyy' format (if it exists)
        if 'orderDate' in st.session_state.data.columns:
            st.session_state.data['orderDate'] = pd.to_datetime(st.session_state.data['orderDate'], format='%Y-%m-%d')

        # Parse the 'time' column (if it exists) in 'HH:MM:SS.fffZ' format
        if 'time' in st.session_state.data.columns:
            st.session_state.data['time'] = pd.to_datetime(st.session_state.data['time'], format='%H:%M:%S.%fZ').dt.time
...
if st.session_state.data is not None:
    ...
    with col3:
        color_class = "positive" if total_revenue_difference_percentage >= 0 else "negative"
        st.markdown(
            f"""
    ...
    # st.markdown("<h4 style='color: green; text-align: center;'>GRN Analysis by products</h4>", unsafe_allow_html=True)

    # # Streamlit UI to upload stock data
    # stock_data = upload_stock_data()

    # # Call GRN analysis if stock data is uploaded

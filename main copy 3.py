...
st.markdown("""
    ...
    /* General web layout */
    .stApp {
        max-width: 80%; /* Use full available width for web */
        margin-left: auto;
        margin-right: auto;
...
if st.session_state.data is not None:
    ...
    st.write("### GRN Analysis")

    # Upload Stock Data
    stock_data = upload_stock_data()

    # Ensure stock data is uploaded
    if stock_data is not None:
        # Perform GRN analysis
        grn_analysis(store_data, stock_data, selected_store)
    else:
        st.warning("Please upload the stock data to perform GRN Analysis.")
    ...
    if st.button("Export PDF"):
            if st.session_state.pdf_exporter.create_pdf("dashboard.pdf"):

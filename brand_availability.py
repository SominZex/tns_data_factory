...
def generate_seaborn_bar_plot(top_n_brands_availability, color_palette):
    """Generate a Seaborn bar plot for exporting to PPT."""
    plt.figure(figsize=(10, 6))
    sns.barplot(data=top_n_brands_availability, x='brandName', y='total_availability', palette=color_palette)
    plt.title('Top Brands by Total Availability (Seaborn Bar Plot)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('top_n_brands_availability_bar.png')
    plt.close()

def generate_seaborn_donut_plot(top_n_brands_availability, color_palette):
    """Generate a Seaborn donut plot for exporting to PPT."""
    plt.figure(figsize=(10, 6))
    wedges, texts, autotexts = plt.pie(
        top_n_brands_availability['total_availability'],
        labels=top_n_brands_availability['brandName'],
        autopct='%1.1f%%',
        startangle=90,
        colors=sns.color_palette(color_palette, len(top_n_brands_availability))
    )
    plt.setp(autotexts, size=10, weight="bold")
    plt.title('Top Brands by Total Availability (Seaborn Donut Plot)')
    plt.axis('equal')
    plt.tight_layout()
    plt.savefig('top_n_brands_availability_donut.png')
    plt.close()

def generate_seaborn_line_plot(top_n_brands_availability, color_palette):
    """Generate a Seaborn line plot for exporting to PPT."""
    plt.figure(figsize=(10, 6))
    sns.lineplot(data=top_n_brands_availability, x='brandName', y='total_availability', marker='o', palette=color_palette)
    plt.title('Top Brands by Total Availability (Seaborn Line Plot)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('top_n_brands_availability_line.png')
    plt.close()

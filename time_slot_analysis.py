...
def time_slot_analysis(store_data, all_data):
    ...
    store_data['time'] = store_data['time'].astype(str)

    # Convert to datetime, coerce errors to NaT
    store_data['time'] = pd.to_datetime(store_data['time'], format='%H:%M:%S', errors='coerce')

    # Check for any invalid times after conversion
    if store_data['time'].isnull().all():
        st.warning("No valid time data available for the store.")
    else:
        # Now that 'time' is a datetime object, extract the hour
    ...
    company_benchmark = pd.read_csv('./company_bechmark/hourly_sales_benchmark.csv')
   ...
   # Group entire dataset by hour for all stores
    ...
    # Convert hours to 12-hour format for display in DataFrame and plots
    sales_by_hour['hour_12'] = sales_by_hour['hour_24'].apply(lambda x: f"{x % 12 or 12} {'AM' if x < 12 else 'PM'}")

    # sales_by_hour = sales_by_hour.drop(columns = ["hour_24"])
    # User input for plot type for Hourly Sales Analysis in the sidebar
    hour_plot_type = st.sidebar.selectbox("Select Plot Type for Hourly Sales Analysis:", ["Line Chart", "Bar Chart"], index=1, key="hour_plot_type")  # Set default to Bar Chart

    # User input for color selection for hour plots in the sidebar
    selected_hour_color = st.sidebar.selectbox("Select Color Scale for Hourly Plot:", color_options_time, key="hour_color_scale")

    # User input to toggle data labels for Hourly Sales Analysis in the sidebar
    ...
    # calculation for daily sales
    # Group sales by day for the selected store
   ...
   # Create the plot based on the selected type for Time Slot Analysis
    ...
    #st.table(sales_display.style.applymap(highlight_negative, subset=['difference']))
    ...
    # Calculate profit for weekly sales
    weekly_sales_sorted['total_profit'] = weekly_sales_sorted['total_sales'] - weekly_sales_sorted['total_cost_price']

    total_store_sales = weekly_sales_sorted['total_sales'].sum()  # Total sales of the selected store
    ...
    if week_plot_type == "Pie Chart":
        ...
        if show_data_labels_week:
            # Move labels outside the pie plot and show both sales and percentage in the labels

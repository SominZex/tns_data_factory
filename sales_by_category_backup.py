...
def sales_by_category_analysis(store_data, all_data):
    ...
    category_sales_company['company_standard'] = np.where(company_total_sales != 0,
        (category_sales_company['company_total_sales'] / company_total_sales * 100).round(2), 0)

    # Merge the company standard into sales_per_category
    sales_per_category = pd.merge(sales_per_category, category_sales_company[['subCategoryOf', 'company_standard']], on='subCategoryOf', how='left')

    # Calculate Variance as the percentage difference between the store contribution and company standard
    sales_per_category['variance'] = np.where(sales_per_category['company_standard'] != 0,
        ((sales_per_category['contribution'] - sales_per_category['company_standard']) / sales_per_category['company_standard'] * 100).round(2), 0)
    ...
    sales_per_category['company_standard'] = pd.to_numeric(sales_per_category['company_standard'], errors='coerce')
    sales_per_category['variance'] = pd.to_numeric(sales_per_category['variance'], errors='coerce')

    sales_per_category["difference"] = sales_per_category['contribution'] - sales_per_category['company_standard']
    ...
    # Convert profit_margin, contribution, company_standard, and variance to string with "%" symbol
    ...
    sales_per_category['company_standard'] = sales_per_category['company_standard'].apply(lambda x: f"{x:.2f}%" if pd.notnull(x) else '')
    ...
    if plot_type_comparison == "Line Chart":
        ...
        fig_comparison.add_trace(go.Scatter(
            x=top_sales_per_category_comparison['subCategoryOf'],
            y=top_sales_per_category_comparison['company_standard'],
            mode='lines+markers',
    ...
    with st.container():
        ...
        final_df = sales_per_category[['subCategoryOf', 'total_sales', 'contribution', 'company_standard', 'variance', 'difference']].style.applymap(

...
def calculate_profits(data, selected_store, start_date, end_date):
    ...
    # Calculate profit per store from filtered data
    store_profit = filtered_data.groupby('storeName').apply(
        lambda x: (x['totalProductPrice'] - x['costPrice']).sum()
    ).reset_index()
    ...
    # Calculate overall average profit (based on existing formula)
    ...
    if selected_store in store_profit['storeName'].values:
        # Calculate profit for the selected store
    ...
    profit_contribution_percentage = (store_profit_value / overall_profit * 100) if overall_profit > 0 else 0
